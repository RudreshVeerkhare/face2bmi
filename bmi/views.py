...
def image_align(image, face_landmarks, output_size=256, transform_size=1024, enable_padding=True):
...
def align(img):
    SHAPE_PRED_MODEL = os.path.join(
        settings.MODEL_DIR, "shape_predictor_68_face_landmarks.dat")
    shape_predictor = dlib.shape_predictor(SHAPE_PRED_MODEL)
    ...
    for detection in dets:
        face_landmarks = [(item.x, item.y)
                          for item in shape_predictor(img, detection).parts()]
...
class call_model(APIView):
    ...
    def post(self, request):
        if request.method == 'POST':
            ...
            response = BmiConfig.predictor.predict(
                preprocess_input(image.astype(np.float))
            )
            print(response)
            # returning JSON response
            return JsonResponse({
                "bmi": str(response[0][0])
            })
